...
def stream(
    ...
    if backend != 'bluemuse':
        ...
        if name == None:
            name = 'Muse'
        if not eeg_disabled:
            eeg_info = StreamInfo(name, 'EEG', MUSE_NB_EEG_CHANNELS, MUSE_SAMPLING_EEG_RATE, 'float32',
                                'Muse%s' % address)
        ...
        if ppg_enabled:
            ppg_info = StreamInfo(name, 'PPG', MUSE_NB_PPG_CHANNELS, MUSE_SAMPLING_PPG_RATE,
        ...
        if acc_enabled:
            acc_info = StreamInfo(name, 'ACC', MUSE_NB_ACC_CHANNELS, MUSE_SAMPLING_ACC_RATE,
                                'float32', 'Muse%s' % address)
        ...
        if gyro_enabled:
            gyro_info = StreamInfo(name, 'GYRO', MUSE_NB_GYRO_CHANNELS, MUSE_SAMPLING_GYRO_RATE,
                                'float32', 'Muse%s' % address)
